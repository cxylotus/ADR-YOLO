...
class VidDetDataset(YOLODataset):
    """Handles video object detection tasks (baseline) by loading annotations from a specified JSON file, supporting YOLO format."""
    
    def __init__(self, *args, task="viddet", json_file, clip_length, **kwargs):
        """Initializes a VidDataset for baseline object detection, loading annotations from a specified JSON file."""
        assert task == "viddet", "only support `viddet` task!"
    ...
    def get_labels(self):
        ...
        cat_ids = coco.getCatIds(catNms=self.data["names"].values())
        ...
        for img_id in TQDM(img_ids):
            ...
            # only extend the sampled imgs
            if "train" in self.json_file:
                if not img_info['is_vid_train_frame']:
                    continue
            ...
            for ann in anns:
                ...
                if box[2] <= 0 or box[3] <= 0:
                    continue
                cls = cat2label[ann['category_id']]  # class
                box = [cls] + box.tolist()
