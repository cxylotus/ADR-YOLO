...
from ultralytics.data.dataset import GroundingDataset, YOLODataset, YOLOMultiModalDataset, VidDetDataset
...
def build_vid_dataset(cfg, img_path, json_file, batch, data, mode="train", rect=False, stride=32, det=False, clip_length=16):
    """Build VID Dataset."""
    dataset = VidDetDataset
    return dataset(
        img_path=img_path,
        json_file=json_file,
        clip_length=clip_length,
        imgsz=cfg.imgsz,
        batch_size=batch if det else 1,
        augment=mode == "train",  # augmentation
        hyp=cfg,  # TODO: probably add a get_hyps_from_cfg function
        rect=cfg.rect or rect,  # rectangular batches
        cache=cfg.cache or None,
